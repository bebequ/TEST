	...
	while (cur_dev) {
		if (cur_dev->vendor_id == vendor_id &&
			...
			if (serial_number) {
				...
				char *interface_component = strstr(cur_dev->path, "&mi_");
				//char *collection_component = strstr(cur_dev->path, "&col");
				//printf("%s\n", path_to_open);
				//if (interface_component && atoi(interface_component + 4) == 1 && collection_component && atoi(collection_component + 4) == 2)
				//	break;
				if (interface_component && interface_num == atoi(interface_component + 4))
				{
					path_to_open = cur_dev->path;
					break;
				}
